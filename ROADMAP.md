...
## v0.95.0 — PID & Version Enrichment (shipped)

- ✅ **`pid` in `/health` and `/health/lite`** — 44 /health fields, 7 /health/lite fields
- ✅ **`rust_version` in `/version`** — 5 /version fields
...
## v1.7.0 — Health Lite Enrichment & Session Context Cleanup (shipped)

- ✅ **`/health/lite` enriched** — added `llm_calls` and `llm_errors` fields (10 fields total, was 8)
- ✅ **`clear_session_context()` wired** — called at all 6 return points in `run_agent_turn` and `run_agent_turn_streaming` (text completion, max rounds, cancellation)
- ✅ **Doctor HTTP endpoint count** — updated from 13 to 14
...
## v1.12.0 — Doctor JSON & CLI Alignment (shipped)

- ✅ **Fix doctor_handler JSON field names** — `check`→`name`, `passed`→`ok` to match CLI expectations
- ✅ **Fix doctor top-level fields** — `status`/`passed`/`total` → `all_ok`/`checks_total`/`checks_passed`
